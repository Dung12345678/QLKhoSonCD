	...
	public partial class frmImportPart : _Forms
	{
		...
		bool SaveData()
		{
			...
			if (MessageBox.Show(String.Format("Bạn có chắc muốn lưu không?"), TextUtils.Caption, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
			{
				if (txbQuantity.Value > 0)
				{
					...
					Expression exp3 = new Expression("IsExported", 0); //Nhập 0 , Xuất 1
					...
					else
					{
						...
						TextUtils.ExcuteProcedure("spImportPartCD",
									new string[] { "@partCode", "@orderCode", "@quantity", "@workerCode", "@partType", "@SonStepID" },
									new object[] { txbPartCode.Text.Trim(), txbOrderCode.Text.Trim(), (int)txbQuantity.Value, txbWorkerCode.Text, type, TextUtils.ToInt(cboCD.SelectedValue) });
					}
		...
		void LoadDataToChart()
		{
			try
			{
				...
				DataTable dt = TextUtils.LoadDataFromSP("spGetSonPartImportDataByDateCD", "A", new string[] { "@CDID", "@StartDate", "@EndDate" }, new object[] { TextUtils.ToInt(cboCD.SelectedValue), startDate, endDate });
		...
		private void txbPartCode_KeyPress(object sender, KeyPressEventArgs e)
		{
			if (e.KeyChar == (char)13)
			{
				//Check chữ T cuối cùng mà CD là gia công thì báo
				if (txbPartCode.Text.Trim().Substring(txbPartCode.Text.Trim().Length - 1).ToUpper() == "T" && cboCD.SelectedIndex == 1)
				{
					MessageBox.Show("Sai mã hàng!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
					txbPartCode.Text = "";
					txbPartCode.Focus();
					return;
				}
